...
function btn_applyFormDownload()
{
	...
	c_ShowProgressBar(3);
	window.open("../../../template/Apply_A1.xlsx","_self");
}
/*
...
/*
名       称: checkTimeBgnEndDate( dt, begin, end)
输入参数: dt, begin, end
输出参数: true（合法）false（不合法）
...
更新时间:
*/
function checkTimeBgnEndDate( dt, begin, end){
	var startTime = (dt + begin).replace( /-/g, '').replace( /:/g, '');
	var endTime = (dt + end).replace( /-/g, '').replace( /:/g, '');
	if(dt != "" && begin != "" && end != ""){
...
//更新时间：
//*/
//function selectZoom(action,width,height) {
//
//	var g_url = "com.mcl.common.ShowModalDialogPanel.flow?sysComm/action="+action;
...
//	try {
//	var topHeight = window.parent.document.body.scrollTop;
//	parent.initMainframe();	
//	window.parent.document.body.scrollTop = topHeight;
//	}catch(e){
//	}
//}
///*
